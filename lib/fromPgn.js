...
class Parser extends EventEmitter {
  constructor (opts, propertyValues) {
    super()
    ...
    if ( propertyValues ) {
      propertyValues.onPropertyValues('canboat-custom-pgns', values => {
        values.filter(v => v != null).forEach(pv => {
          pv.value.PGNs.forEach(pgn => {
            if ( !addCustomPgn(pgn, this.options.allowCustomPGNOverride, pv.value.callback) ) {
  ...
  _parse(pgn, bs, len, coalesced, cb, sourceString) {
    ...
    const customPgn = getCustomPgn(pgn.pgn)
    let pgnList = customPgn ? customPgn.definitions : pgns[pgn.pgn]
    ...
    try {
      ...
      this.emit('pgn', pgn)
      cb && cb(undefined, pgn)
      if ( customPgn && customPgn.callback ) {
        customPgn.callback(pgn)
      }
      return pgn
    } catch ( error ) {
