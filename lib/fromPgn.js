...
class Parser extends EventEmitter {
  ...
  _parse(pgn, bs, len, coalesced, cb, sourceString) {
    ...
    //} else if ( pgnData.Length > 0x8 || (len == 0x8 && (pgnData.RepeatingFields || couldBeMulti))) {
      ...
      var packet = this.devices[pgn.src][pgn.Id]

      if ( !packet ) {
        packet = { bufferSize:0, lastPacket: 0, src: [] }
        this.devices[pgn.src][pgn.Id] = packet
      }
      if ( sourceString ) {
      ...
      if ( bucket == 0 ) {
        ...
        // TODO : cb && cb(`Could not parse ${JSON.stringify(pgn)}`)
        bs.byteIndex = start
        delete this.devices[pgn.src][pgn.Id]
        return
      } else {
    ...
    try {
      ...
      for ( var i = 0; i < fields.length-RepeatingFields; i++ ) {
        ...
        if ( hasMatch ) {
          //console.log(`looking for ${field.Id} == ${value}`)
      ...
      if ( RepeatingFields > 0 ) {
        ...
        if ( pgnData.RepeatingFieldSet1CountField !== undefined ) {
          count = pgn.fields[pgnData.Fields[pgnData.RepeatingFieldSet1CountField-1].Id]
        } else {
          count = 2048
        }
...
function readField(options, runPostProcessor, pgn, field, bs, fields) {
  ...
  //console.log(`${field.Id} ${value} ${field.Resolution}`)
  ...
  if ( value != null && !_.isUndefined(value) ) {
    ...
    if ( postProcessor ) {
      ...
      if ( options.checkForInvalidFields !== false && max !== 'undefined' &&
           ...
           field.BitLength > 1 &&
           max - value <= 0 ) {
        //console.log(`Bad field ${field.Id} ${max - value}`)
        value = null
...
/*
      // TODO : TAMA 2025-07-10 : Check if it's better to remove this parser, i can always parse it elsewhere if i need to
      ...
      // TODO : TAMA 2025-07-10 : Same as above
      if ( field.Id === 'industryCode' && _.isNumber(value) && runPostProcessor ) {
...
function readVariableLengthField(options, pgn, field, bs) {
  ...
  try {
    var refField = getField(pgn.fields.pgnData, bs.view.buffer[bs.byteIndex-1]-1, pgn)
