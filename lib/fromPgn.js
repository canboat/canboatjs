...
const debug = require('debug')('canbusjs:fromPgn')
...
function organizedPGNs() {
  ...
  pgns.PGNs.forEach(pgn => {
    ...
    pgn.fields = _.isArray(pgn.Fields) ? pgn.Fields : pgn.Fields ? [pgn.Fields] : []
...
x = function(timestamp, prio, pgn, src, dst) {
  return {
    timestamp: timestamp,        prio: prio,        pgn: pgn,        src: src,        dst: dst,
    fields: {
      "Control": null,"Group Function": null,"Reserved": null,"PGN": null
    }
  }
}
...
class Parser extends EventEmitter {
  ...
  _parse(pgn, bs, len) {
    ...
    if ( len > 0x8 || this.format == FORMAT_FAST ) {
  ...
  parseString (pgn_data) {
    if (typeof pgn_data === 'string' && pgn_data.trim().length > 0) {
      ...
      var pgn = pgnData[0].create(...split)
...
fieldTypePostProcessors['Date'] = (field, value) => {
  if ( value >= 0xfffd ) {
    ...
    value = `${1900 + date.getYear()}.${pad2(date.getMonth()+1)}.${pad2(date.getDate())}`
  }
  return value
}
