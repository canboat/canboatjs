...
class Parser extends EventEmitter {
  ...
  isN2KOver0183(sentence) {
    return sentence.startsWith('$PCDIN,') || sentence.startsWith('$MXPGN,')
  }

  parseN2KOver0183(sentence) {
    if ( sentence.startsWith('$PCDIN,') ) {
      this.parsePCDIN(sentence)
      return true
    } else if ( sentence.startsWith('$MXPGN,') ) {
      this.parseMXPGN(sentence)
      return true
    }
    return false
  }
  ...
  parseMXPGN(sentence) {
    ...
    for ( var i = data.length-2, j = 0; i >= 0; i -= 2, j++ ) {
      console.log('parse: ' + data.slice(i, i+2))
