...
class Parser extends EventEmitter {
  ...
  _parse(pgn, bs, len, coalesced, cb) {
  ...
  parse(data, cb) {
    if (_.isString(data) ) {
      ...
      return this.parsePgnData(data.pgn, data.length, data.data, true, cb)
    }
  }
  ...
  parseYDGW02(pgn_data, cb) {
    try {
      ...
      if (!error && direction === 'R') {
        ...
        delete pgn.format
        const res = this._parse(pgn, bs, data.length, false, cb)
        if ( res ) {
  ...
  parseString (pgn_data, cb) {
    try {
      ...
      delete pgn.type
      delete pgn.prefix
      const res = this._parse(pgn, bs, len || data.length, coalesced, cb)
      if (res) {
        debug('parsed pgn %j', pgn)
      }
      return res
