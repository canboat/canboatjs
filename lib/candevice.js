...
function sendPGNList(device, src) {
  const transmitPGNs = !device.options.app.getTransmitPGNs ? defaultTransmitPGNs : device.options.app.getTransmitPGNs()
  debug('sending 126464 with pgns: %j', transmitPGNs)
  const pgnList = {
    pgn: 126464,
    dst: src,
    "Function Code": 0,
