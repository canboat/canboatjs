...
const { encodeActisense, parseActisense } = require('./stringMsg')
...
function pgnToActisenseSerialFormat(pgn) {
  return encodeActisense(pgn.pgn, toPgn(pgn), pgn.dst)
}
...
function toYdgwRawFormat(pgn, data, dst=255) {
  //canId data
...
function pgnToYdgwRawFormat(pgn) {
  return toYdgwRawFormat(pgn, toPgn(pgn), pgn.dst)
}

function actisenseToYdgwRawFormat(msg) {
  const pgn = parseActisense(msg)
  const canId = encodeCanId(pgn)
