...
const { getPlainPGNs, getManufacturerCode, getIndustryCode, getCanIdFromYdwgPGN , actisenseSerialToBuffer} = require('./utilities')
...
function toPgn(data) {
  ...
  for ( var index = 0; index < fields.length - pgnData.RepeatingFields; index++ ) {
    ...
    writeField(bs, field, data, value)
  }
...
function writeField(bs, field, data, value, bitLength) {
...
function writeVariableLengthField(bs, pgn, field, value) {
  ...
  if ( refField ) {
    ...
    return writeField(bs, refField, pgn, value, bits)
  }
}
...
function actisenseToYdwgRawFormat(msg) {
...
module.exports.actisenseToYdwgRawFormat = actisenseToYdwgRawFormat
