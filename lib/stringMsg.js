...
exports.parseYDRAW = (input) => {
  ...
  return {
    ...parseCanId(parseInt(canId, 16)),
    data: Buffer.from(data.join(''), 'hex'),
    direction,
    format: 'YDRAW',
    timestamp: parseDate(time, 'HH:mm:ss.SSS', new Date()),
  }
}
...
exports.parsePCDIN = (input) => {
  ...
  return {
    data: Buffer.from(rmChecksum(data), 'hex'),
    dst: 255,
    format: 'PCDIN',
    pgn: parseInt(pgn, 16),
    prefix,
...
exports.parsePDGY = (input) => {
  const parts = input.split(',')
  if ( split.length !== 7 ) return { error: true, input }
...
exports.parseCandump1 = (input) => {
  ...
  return {
    ...parseCanId(parseInt(trimWrap(canId), 16)),
    data: Buffer.from(data.join(''), 'hex'),
    format: 'candump1',
