...
const { arrBuff, byteString, rmChecksum, trimWrap } = require('./utilities')
...
const buildErr = curry((format, msg, input) => ({
  error: new Error(msg), format, input,
}))
...
exports.parseYDRAW = (input) => {
  ...
  if ( parts.length != 11 ) return buildErr('YDRAW', 'Invalid parts.', input)
  const [ time, direction, canId, ...data ] = parts
  return buildMsg(
    parseCanIdStr(canId), 'YDRAW', arrBuff(data),
    { direction, timestamp: parseDate(time + 'Z', 'HH:mm:ss.SSSX', new Date()) }
  )
}
...
exports.parsePDGY = (input) => {
  const [ prefix, pgn, prio, src, dst, timer, data ] = input.split(',')
  return buildMsg(
