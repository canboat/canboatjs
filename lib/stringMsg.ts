 ...
 */
function buildMsg(_canIdInfo:any, format:string, data:Buffer, rest:any = {}) {
...
const buildErr = (format:string, msg:string, input:string|undefined = undefined) => {
  return {
...
export const isActisense = (input:string) => (input.charAt(10) === 'T' && input.charAt(23) === 'Z') || (input.charAt(10) === '-' && input.charAt(23) === ',')
...
export const encodeActisense = ({
  pgn, data, timestamp, prio = 2, dst = 255, src = 0 }:any) => ([
    timestamp || new Date().toISOString(),
    prio, pgn, src, dst, data.length,
    byteString(data)
  ].join(','))

export const toActisenseSerialFormat = (pgn:any, data:any, dst=255, src=0, prio=2) => exports.encodeActisense({
...
export const isActisenseN2KASCII = (input:string) => input.charAt(0) === 'A' && input.charAt(7) === '.' && input.charAt(11) === ' '
...
export const encodePCDIN = ({ prefix = '$PCDIN', pgn, data, dst = 255}:any) => {
...
export const parseMXPGN = (input:string, options:any|undefined = undefined) => {
  const sentence = get0183Sentence(input)
  if ( sentence ) {
...
export const encodeMXPGN = ({ prefix = '$MXPGN', pgn, prio, src, data }:any) => {
  if (src > 255) src = 255;
  if (!prio) prio = 3;
  if (!src) src = 255;
...
export const isN2KOver0183 = (msg:string) => { return isPCDIN(msg) || isMXPGN(msg) }

export const parseN2KOver0183 = (msg:string) => { return parsePCDIN(msg) || parseMXPGN(msg) }
