...
describe('buildMaretronConfigCommand', () => {
  test('SIM100 Read All produces correct payload', () => {
    ...
    const expected = [
      0x01, 0x00, 0xef, 0x01, 0xf8, 0x04, 0x01, 0x89, 0x98, 0x02, 0x33,
      0x5c, 0x03, 0x01, 0x00, 0x56, 0xff
    ]
  ...
  test('SIM100 Write Ch3 mode produces correct payload', () => {
    ...
    const result = buildMaretronConfigCommand(211, 23603, [0x57, 0x22, 0x01, 0xff])
    const bytes = result.fields.data
      .split(',')
      .map((s: string) => parseInt(s, 16))
    const expected = [
      0x01, 0x00, 0xef, 0x01, 0xf8, 0x04, 0x01, 0x89, 0x98, 0x02, 0x33,
      0x5c, 0x03, 0x01, 0x00, 0x57, 0x22, 0x01, 0xff
    ]
    expect(bytes).toEqual(expected)
  })
