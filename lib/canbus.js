  ...
  if ( this.candevice ) {
    ...
    if ( this.socketCanWriter ) {
      ...
      if ( _.isObject(msg) ) {
        ...
        var pgn = actisenseSerialToBuffer(msg)
        canid = getCanIdFromPGN(pgn)
        buffer = pgn.data
        console.log('converted string to %j', pgn)
      }
...
function getPlainPGNs(buffer) {
  ...
  for ( var index = 6; index < buffer.length; index += 8 ) {
    var next = new Buffer(8)
    next.writeUInt8(bucket++, 0)
    buffer.copy(next, 1, index, index+7)
    res.push(next)
  }
  return res
}
