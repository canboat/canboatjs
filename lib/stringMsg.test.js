...
describe('parseN2kString', () => {
  test('empty string or not string', () => {
    ...
    const errMsg = { error: true, input: '', message: emptyMsg }
    expect(parseN2kString('')).toEqual(errMsg)
    expect(parseN2kString(5)).toEqual({ ...errMsg, input: 5 })
  })
  test('parser not found', () => {
    const emptyMsg = 'Parser not found for input.'
    const errMsg = { error: true, input: 'foo,bar', message: emptyMsg }
